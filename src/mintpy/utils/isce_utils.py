...
def extract_multilook_number(geom_dir, meta=dict(), fext_list=['.rdr','.geo','.rdr.full','.geo.full']):
    ...
    # Fallback: read ALOOKS/RLOOKS from ISCE2 XML properties
    # (generated by sarforge / dolphin with numberAzimuthLooks / numberRangeLooks)
    if 'ALOOKS' not in meta:
        for fbase in ['hgt','lat','lon','los','shadowMask','incLocal']:
            for fext in fext_list:
                fname = os.path.join(geom_dir, fbase+fext)
                xml_file = fname + '.xml'
                if os.path.isfile(xml_file):
                    try:
                        ...
                        root = tree.getroot()
                        alooks_elem = root.find(".//property[@name='numberAzimuthLooks']/value")
                        rlooks_elem = root.find(".//property[@name='numberRangeLooks']/value")
                        if alooks_elem is not None and rlooks_elem is not None:
                            alooks = int(alooks_elem.text)
                            rlooks = int(rlooks_elem.text)
                            if rlooks > 1 or alooks > 1:
                                meta['ALOOKS'] = alooks
                                meta['RLOOKS'] = rlooks
                                print(f'read ALOOKS={alooks} RLOOKS={rlooks} from {xml_file}')
                                break
                    except Exception:
                        pass
            if 'ALOOKS' in meta:
