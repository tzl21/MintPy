...
Interferogram products (``intf_dir``):
    isce2:  ``intf_dir/date_pair/xxx.int``
            ...
            ``intf_dir/date_pair/xxx.cpx.coh``        (complex coherence)
            ``intf_dir/date_pair/xxx.phsig.coh``      (phase-sigma coherence)
    isce3:  same with ``.tif`` appended: ``xxx.int.tif``, ``xxx.unw.tif``,
            ``xxx.unw.conncomp.tif``, ``xxx.cpx.coh.tif``, ``xxx.phsig.coh.tif``
...
def coh_ext(processor: str, kind: str = COH_KIND_PHSIG) -> str:
    ...
    Complex coherence uses the ``.cpx`` infix, phase-sigma the ``.phsig``
    infix: ``xxx.cpx.coh[.tif]`` / ``xxx.phsig.coh[.tif]``.
    """
...
def strip_product_extensions(name: str, processor: str) -> str:
    ...
    ``fullres.int.tif`` -> ``fullres``; ``mli.phsig.coh`` -> ``mli``
    ...
    for infix in (".cpx", ".phsig", ATM_INFIX):
