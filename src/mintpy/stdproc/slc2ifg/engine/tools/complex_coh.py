...
class ComplexCohTool(Tool):
    ...
    outputs = [Port('coh', 'file', variant='fullres', ext='_cpx.coh[.tif]')]
    ...
    def run(self, ctx: ToolContext) -> Dict[str, Path]:
        ...
        from mintpy.stdproc.slc2ifg.generate_coh_complex import (
        ...
        if tile_size > 0:
            from mintpy.stdproc.slc2ifg.engine.tiling import complex_coh_tiled
