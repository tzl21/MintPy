...
class Stage:
    ...
    core : bool
        ...
        run when they are in ``engine.tools``/``AUTO_TOOLS`` or listed in
        ``engine.stages`` — nothing is force-enabled).
    optional : bool
        ``True`` means disabled in the default chain; enabled only when
        listed in the ``engine.tools`` whitelist or explicitly in
...
DEFAULT_CHAIN: List[Stage] = [
    Stage('crop_slc', scope='source', optional=True,
          desc='Crop SLC to a geographic bbox (requires slc2ifg.crop_slc.wsen)'),
...
def resolve_chain(
    ...
    stages_cfg : list of str or None
        ...
        chain.
    tools : list of str
        ``engine.tools`` whitelist (already includes the special toggling
    ...
    # ``engine.tools``.  Nothing is force-enabled: e.g. phsig_coh is computed
    # only when explicitly requested, and AUTO_TOOLS defines what
    # ``engine.tools = auto`` runs.
