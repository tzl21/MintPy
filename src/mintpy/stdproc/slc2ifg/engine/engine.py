...
Builds a :class:`~mintpy.stdproc.slc2ifg.engine.dag.TaskGraph` from an
:class:`~mintpy.stdproc.slc2ifg.engine.config.EngineConfig`, covering the same phases as
...
from mintpy.stdproc.slc2ifg.engine.chain import resolve_chain
from mintpy.stdproc.slc2ifg.engine.config import EngineConfig
from mintpy.stdproc.slc2ifg.engine.dag import TaskGraph, TaskNode
from mintpy.stdproc.slc2ifg.engine.manifest import (
    Manifest,
    execute_cleanup,
    plan_cleanup,
)
from mintpy.stdproc.slc2ifg.engine.scheduler import execute_graph
...
def _legacy_opt(cfg, new_key: str, legacy_key: str, kind: str,
    ...
    """
    from mintpy.stdproc.slc2ifg.engine.config import (
        get_float_opt,
        get_int_opt,
        get_opt,
        has_real_value,
...
class Engine:
    ...
    def build_graph(self) -> TaskGraph:
        ...
        # whenever the user's selection omits generate_ifgram: either an
        # explicit engine.stages without it, or an engine.tools whitelist
        # that leaves it out.
        ...
        if entry_mode:
            ...
            if not chain:
                raise ValueError(
                    ...
                    "in engine.tools/stages) but no processing stage remains — "
        ...
        if multi:
            if 'stitch' not in chain_names:
                raise ValueError(
                    ...
                    "(add 'stitch' to engine.stages / engine.tools)")
    ...
    def _entry_variant(self) -> str:
        ...
        from mintpy.stdproc.slc2ifg.utils.naming import IFG_VARIANTS
    ...
    def _auto_tile_size(self) -> Optional[int]:
        ...
        """
        from mintpy.stdproc.slc2ifg.engine.config import get_int_opt
        from mintpy.stdproc.slc2ifg.engine.resources import gpu_memory_gb
        try:
    ...
    def _apply_gpu(self, g: TaskGraph) -> None:
        ...
        :func:`mintpy.stdproc.slc2ifg.engine.tool.gpu_tools`); when the backend is
    ...
    def _run_ifgram_list(self, slc_dir: Path, out_dir: Path) -> Path:
        ...
        cfg = self.config.raw
        from mintpy.stdproc.slc2ifg.engine.config import get_int_opt, get_opt
    ...
    def _ps_nlks(self) -> float:
        cfg = self.config.raw
        from mintpy.stdproc.slc2ifg.engine.config import get_float_opt, get_int_opt
