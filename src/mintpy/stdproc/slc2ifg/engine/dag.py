...
- ``*_cpx.coh`` products may only exist for the ``fullres`` variant;
...
class TaskGraph:
    ...
    def _variant_of_path(path: Any, node: TaskNode) -> str:
        ...
        which ``mintpy.stdproc.slc2ifg`` pulls in at import time).
        """
        name = Path(str(path)).name
        processor = node.ctx.params.get('processor', 'isce3')
        try:
            from mintpy.stdproc.slc2ifg.utils.naming import variant_of
