...
Fixed output structure (see ``utils.naming``):
    ``output_dir/{date1}_{date2}/xxx.phsig.coh``      (isce2)
    ``output_dir/{date1}_{date2}/xxx.phsig.coh.tif``  (isce3)
...
def _output_path(input_file, output_dir, processor):
    ...
    ``output_dir/{date1}_{date2}/{variant}.phsig.coh[.tif]`` when the input
    lives in a date-pair directory; ``output_dir/{base}.phsig.coh[.tif]``
