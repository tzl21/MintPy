...
# ---------------------------------------------------------------------------
# isce2 geometry cropping (kept in ENVI + .hdr / .xml on purpose)
# ---------------------------------------------------------------------------
...
def crop_geocoded(src, out_file, wsen, subdataset='/data/VV', buffer=0.0,
...
def crop_slc(input_dir, output_dir, bbox, processor='isce3', pattern=None,
             buffer=0.0, geom_dir=None, prefix='', by_burst=False,
             file_list=None, subdataset='/data/VV', workers=1,
             no_skip_existing=False, no_burst_dirs=False,
             dest_epsg=None, fill_nan=False, compress_level=6,
             dry_run=False) -> int:
    """Crop SLCs to a geographic bbox (both processors).
    ...
    output_dir : str
        Directory for the cropped SLCs (``[prefix]yyyymmdd.slc.tif``).
    bbox : tuple of 4 floats
        (west, south, east, north) in EPSG:4326.
    processor : {'isce3', 'isce2'}
    pattern : str, optional
        Geocoded input glob (isce3); defaults to the processor's raw SLC pattern.
    buffer : float
        Extra margin in degrees around ``bbox``.
    geom_dir : str, optional
        ISCE2 full-resolution geometry directory (``lat.rdr.full`` / ``lon.rdr.full``);
        required for isce2.
    ...
    if processor == 'isce2' and not geom_dir:
        raise ValueError('geom_dir is required for isce2 (radar-coordinate) cropping')
    ...
    output_files = _output_paths(input_files, output_dir, prefix=prefix,
    ...
    if processor == 'isce2':
        crop_window = find_crop_window_from_full_files(
            os.path.join(geom_dir, 'lon.rdr.full'),
            os.path.join(geom_dir, 'lat.rdr.full'),
            _expand(bbox, buffer),
        )
        output_geom_dir = os.path.join(os.path.dirname(os.path.abspath(output_dir)), 'geom')
        coord_results = crop_coordinate_files(
            geom_dir, crop_window, output_geom_dir,
    ...
    def _work(task):
        ...
        try:
            if processor == 'isce2':
                return (src, out, crop_radar(src, out, crop_window, fill_nan=fill_nan), '')
