...
def update_attribute4resize(atr_in, resize2shape, print_msg=True):
    ...
    vprint('LENGTH, WIDTH ',atr['LENGTH'], atr['WIDTH'])
    ...
    #     data = {}
    #     with open(MiaplpyConfig, "r") as f:
    #         for line in f:
    #             line = line.strip()
    #             if not line:
    #                 continue
    ...
    #     yscale,xscale = 1,1
    #     atr['ALOOKS'] = np.rint(int(data.get('NLOOKSAZ', 1))).astype(int)
    #     atr['RLOOKS'] = np.rint(int(data.get('NLOOKSRANG', 1))).astype(int)
    #     atr['NCORRLOOKS'] = np.rint(int(data.get('NCORRLOOKS', 1))).astype(int)
    #     f.close()
    #     vprint('Miaplpy update')
