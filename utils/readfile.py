...
def read_gdal_vrt(fname):
    ...
    meta_ext = os.path.splitext(fname)[1].lower()
    if any(fname.lower().endswith(ext) for ext in ['.int.tif','.int.cor.tif','.unw.tif','.unw.conncomp.tif']):
        date12_str = ptime.get_date12_from_path(fname)
        if date12_str is not None:
            atr['DATE12'] = date12_str
