...
def load_metadata_directly(xml_file):
    """
    Load metadata from .burst.xml file and compute values as in extract_tops_metadata.
    
    Parameters:
        xml_file : str, path of the .burst.xml file
    
    Returns:
        meta : dict, metadata matching the structure of extract_tops_metadata output
    """
    # Parse XML file
    import isce
    import isceobj
    ...
    attrs_elem = root.find('burst_attributes')
    if attrs_elem is None:
        raise ValueError("No burst_attributes found in XML")
    
    # Initialize metadata dictionary
    meta = {}
    
    # Helper function to parse XML values based on type
    def parse_xml_value(elem):
        value_str = elem.text
        type_str = elem.get('type', 'str')
        ...
        if value_str == 'None':
            ...
            return np.fromstring(value_str.strip('[]'), sep=' ')
        else:
            return value_str
    ...
    for elem in attrs_elem:
        key = elem.tag
        burst_attrs[key] = parse_xml_value(elem)
    
    # Map XML attributes to meta dictionary (matching original function)
    # Direct mapping from XML to meta
    meta['prf'] = burst_attrs.get('prf')
    meta['burstStartUTC'] = burst_attrs.get('burstStartUTC')
    meta['burstStopUTC'] = burst_attrs.get('burstStopUTC')
    meta['radarWavelength'] = burst_attrs.get('radarWavelength')
    meta['startingRange'] = burst_attrs.get('startingRange')
    meta['passDirection'] = burst_attrs.get('passDirection')
    meta['polarization'] = burst_attrs.get('polarization')
    meta['trackNumber'] = burst_attrs.get('trackNumber')
    meta['orbitNumber'] = burst_attrs.get('orbitNumber')
    
    # Additional attributes needed for calculations
    sensing_mid = burst_attrs.get('sensingMid')
    azimuth_time_interval = burst_attrs.get('azimuthTimeInterval')
    range_pixel_size = burst_attrs.get('rangePixelSize')
    swath_number = burst_attrs.get('swathNumber')
    ascending_node_time = burst_attrs.get('ascendingNodeTime')
    satellite_speed = burst_attrs.get('satelliteSpeed')
    first_frame_number = burst_attrs.get('firstFrameNumber')
    last_frame_number = burst_attrs.get('lastFrameNumber')
    
    # Set platform (assuming Sentinel-1)
    meta['PLATFORM'] = 'sen'
    ...
    if satellite_speed is not None and azimuth_time_interval is not None:
        meta['satelliteSpeed'] = satellite_speed
        meta['azimuthPixelSize'] = satellite_speed * azimuth_time_interval
    else:
        meta['satelliteSpeed'] = 7598.0
        meta['azimuthPixelSize'] = 0.0
    
    # Range pixel size
    if range_pixel_size is not None:
        meta['rangePixelSize'] = range_pixel_size
    else:
        meta['rangePixelSize'] = 0.0
    
    # Sentinel-1 TOPS spatial resolution (simplified - would need sensor module)
    # These are typical values for Sentinel-1 IW mode
    iw_str = 'IW2'
    if os.path.basename(xml_file).startswith('IW'):
        iw_str = (os.path.basename(xml_file))[:-len('.burst.xml')]
    ...
    position = burst_attrs.get('position')

    elp = Planet(pname='Earth').ellipsoid
    llh = elp.xyz_to_llh(position)
    meta['HEADING'] =  burst_attrs.get('HEADING')
    elp.setSCH(llh[0], llh[1], meta['HEADING'])
    ...
    if swath_number is not None:
        meta['swathNumber'] = swath_number
    else:
        meta['swathNumber'] = '2'
    
    # Calculate ASF frame number for Sentinel-1
    if (ascending_node_time is not None and 
        meta['burstStartUTC'] is not None and 
        meta['burstStopUTC'] is not None):
        meta['firstFrameNumber'] = int(0.2 * (meta['burstStartUTC'] - ascending_node_time).total_seconds())
        meta['lastFrameNumber'] = int(0.2 * (meta['burstStopUTC'] - ascending_node_time).total_seconds())
    elif first_frame_number is not None and last_frame_number is not None:
        # Use pre-calculated values from XML
        meta['firstFrameNumber'] = first_frame_number
        meta['lastFrameNumber'] = last_frame_number
    else:
        meta['firstFrameNumber'] = 0
        meta['lastFrameNumber'] = 0
    
    # Add aliases for compatibility with original function
    meta['startUTC'] = meta.get('burstStartUTC')
    meta['stopUTC'] = meta.get('burstStopUTC')
...
def extract_tops_metadata(xml_file):
    ...
    if xml_file.lower().endswith('.burst.xml'):
        return load_metadata_directly(xml_file)
...
def read_tops_baseline(baseline_file):
    ...
    with open(baseline_file) as f:
        for line in f:
            ...
            if "bperp (average)" in c[0].lower() or "bperp average (m)" in c[0].lower():
