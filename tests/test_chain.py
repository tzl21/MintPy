...
def _entry_engine(tools: str = 'phsig_coh,unwrap',
    ...
    Mid-chain entry always reads the unified ``<work_dir>/ifgrams`` tree; the
    input variant is auto-detected from the ``*.int.tif`` files present, so a
    dummy ``{variant}.int.tif`` is created per date pair.
    """
    import tempfile
    ...
    for d1, d2 in (('20220105', '20220117'), ('20220117', '20220210')):
        pair_dir = inp / f'{d1}_{d2}'
        pair_dir.mkdir(parents=True, exist_ok=True)
        (pair_dir / f'{variant}.int.tif').touch()
...
def test_entry_complex_coh_from_slcs():
    ...
    assert n.ctx.inputs['slc_dir'] == eng.slc.input_dirs()
...
def test_entry_complex_coh_slc_pairs_fallback():
    ...
    assert eng._entry_slcs_available()          # inferred from the SLC files
    g = eng.plan(dry_run=True)
    assert sorted(g.nodes) == [
        'complex_coh#20220105_20220117',
...
def test_entry_select_infers_slc_pattern():
    """The engine's select params carry the SLC pattern inferred from slc_input.

    There is no slc2ifg.slc_pattern key anymore; the pattern is derived from
    the files under slc_input (here an OPERA ``<date>/tXXX_....h5`` layout).
    """
    import tempfile
    ...
    assert p['slc_pattern'] == '*.h5'
    assert p['coh_root'] == str(eng.ifgram_dir)
    assert 'weight_source' not in p
