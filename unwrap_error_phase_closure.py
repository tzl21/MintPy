...
def calc_num_triplet_with_nonzero_integer_ambiguity(ifgram_file, mask_file=None, dsName='unwrapPhase',
    ...
    if out_file is None:
        ...
        #
        out_file_norm = 'numTriNonzeroIntAmbiguity_norm'
        out_file_norm += f'4{dsName}' if dsName != 'unwrapPhase' else ''
        out_file_norm += '.h5'
        out_file_norm = os.path.join(out_dir, out_file_norm)
        #
    ...
    #
    num_nonzero_closure_norm = num_nonzero_closure/C.shape[0]
    print('write to file', out_file_norm)
    writefile.write(
        num_nonzero_closure_norm,
        out_file=out_file_norm,
        metadata=meta,
    )
    #
    ...
    #
    pp.plot_num_triplet_with_nonzero_integer_ambiguity(out_file_norm,scale=C.shape[0])
    #
    return out_file
